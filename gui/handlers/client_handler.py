...
class ClientHandler:
    ...
    def buscar_cliente(self):
        ...
        if not permitir_ingreso(rfc, "rfc"):
            QMessageBox.warning(
                ...
                "Valores no validos como RFC, ingrese valores validos",
            )
            return
        ...
        if resultado is None:
            ...
            QMessageBox.information(
                ...
                f"No se encontró ningún cliente con el RFC: {self.navegacion.reRfc.text()}",
            )
            self.navegacion.cliente_info.setText(
                f"No se encontro ningun cliente con el RFC: {self.navegacion.reRfc.text()}, debes registrarlo."
            )
        else:
    ...
    def tarjeta_cliente(self, cliente):
        ...
        label_nombre.setStyleSheet(
            "color: rgb(0, 0, 0); border-radius: 5px; border-bottom: 3px solid rgba(155, 88, 43, 1.0); border-right: 3px solid  rgba(155, 88, 43, 1.0);"
        )
