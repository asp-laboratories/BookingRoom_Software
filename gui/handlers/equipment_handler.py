...
class EquipmentHandler:
    ...
    def registrar_equipamiento(self, nombre, descripcion, costo, stock, tipo):
        try:
            resultado = self.equipamiento.registrar_equipamento(nombre, descripcion, costo, stock, tipo)
    ...
    def actualizar_equipamiento(self, campo: str, id_busqueda: int, nuevo_valor: str):
        try:
            resultado = self.equipamiento.actualizar_equipamento(campo, id_busqueda, nuevo_valor)
            if not resultado:
                QMessageBox.critical(
                    None, 
    ...
    def desplegar_informacion_equipamiento(self):
        ...
        try:
            resultado = self.equipamiento.listar_equipamentos_informacion(int(self.navegacion.slIngresarBusqueda_5.text()))
            ...
            QMessageBox.warning(None, "Datos Inválidos", "El numero de equipamiento debe ser un número entero válido.")
        except TypeError:
            QMessageBox.warning(None, "Datos Inválidos", "El numero de equipamiento debe ser un número entero válido.")
    ...
    def eliminar_equipamiento(self, id_equipamiento: int):
        try:
            ...
            if not resultado:
                QMessageBox.critical(None, "Error de Eliminación", "No se pudo eliminar el equipamiento. Verifique que el numero.")
            else:
                QMessageBox.information(None, "Eliminación Exitosa", f"El equipamiento {id_equipamiento} ha sido eliminado correctamente.")
        except Exception as e:
            QMessageBox.critical(None, "Error Inesperado", f"Ocurrió un error grave de base de datos durante la eliminación: {e}")
    ...
    def buscar_tipo_equipo(self):
        ...
        resultado = self.equipamiento.listar_equipamiento_tipo(descripcion)
        if resultado:
            mensaje = ""
    ...
    def actualizar_estado_equipa(self, num_equipo: int, estado_nuevo: str, estado_origen: str, cantidad: int):
        try:
            ...
            if resultado:
                QMessageBox.information(None, "Actualización Exitosa", f"Se han movido {cantidad} unidades del equipo {num_equipo} al estado '{estado_nuevo}'.")
            else:
                QMessageBox.critical(None, "Error de Actualización", f"No se pudo actualizar el estado del equipo {num_equipo}. Verifique la existencia del numero, la cantidad disponible en el estado origen o si los estados son válidos.")
        except Exception as e:
            QMessageBox.critical(None, "Error Inesperado", f"Ocurrió un error de conexión/base de datos: {e}")
    ...
    def mostrar_controles_cantidad(self):
        ...
        for equipamiento_item in equipamientos_seleccionados:
            ...
            if equipa_data is None: continue
    ...
    def crear_control_cantidad(self, nombre, costoRenta, numEquipa):
        ...
        if nombre not in self.controles_equipos:
            self.controles_equipos[nombre] = {'label_cantidad': lbl_cantidad, 'label_subtotal': lbl_subtotal, 'costo': costoRenta}
        sub = QLabel("Subtotal: ")
    ...
    def intentar_registrar_equipamiento(self):
        try:
            ...
            if self.main_window.mostrar_confirmacion("Confirmar Registro de Equipamiento", f"¿Deseas registrar el equipo '{nombre_equipo}' (Costo: ${costo_renta}, Stock: {stock})?"):
                self.registrar_equipamiento(nombre_equipo, descripcion, costo_renta, stock, tipo)
            else:
                QMessageBox.information(None, "Registro Cancelado", "La operación de registro ha sido cancelada.")
        except ValueError:
            QMessageBox.warning(None, "Datos Inválidos", "Asegúrate de que 'Costo de Renta' y 'Stock' contengan valores numéricos válidos.")
        except Exception as e:
            QMessageBox.critical(None, "Error de Pre-Validación", f"Ocurrió un error al procesar los datos: {e}")
    ...
    def intentar_actualizar_equipamiento(self):
        try:
            ...
            if self.main_window.mostrar_confirmacion("Confirmar Actualización de Equipamiento", f"¿Deseas cambiar el campo '{campo}' del equipo con ID {id_busqueda} al valor '{nuevo_valor}'?"):
                self.actualizar_equipamiento(campo, id_busqueda, nuevo_valor)
            else:
                QMessageBox.information(None, "Actualización Cancelada", "La operación de actualización del equipamiento ha sido cancelada.")
        except ValueError:
            QMessageBox.warning(None, "Datos Inválidos", "Asegúrate de que la búsqueda contenga un valor numérico entero válido.")
        except Exception as e:
            QMessageBox.critical(None, "Error de Pre-Validación", f"Ocurrió un error al procesar los datos: {e}") 
    ...
    def intentar_eliminar_equipamiento(self):
        try:
            ...
            if self.main_window.mostrar_confirmacion("Confirmar Eliminación de Equipamiento", f"⚠️ **Advertencia:** ¿Estás seguro de ELIMINAR el equipo: {id_equipo_a_eliminar}? Esta acción es irreversible."):
                self.eliminar_equipamiento(id_equipo_a_eliminar)
            else:
                QMessageBox.information(None, "Operación Cancelada", "La eliminación del equipamiento ha sido cancelada por el usuario.")
        except ValueError:
            QMessageBox.warning(None, "Datos Inválidos", "El equipamiento a eliminar debe ser un número entero válido.")
        except Exception as e:
            QMessageBox.critical(None, "Error de Pre-Validación", f"Ocurrió un error al intentar leer el ID: {e}")
    ...
    def intentar_actualizar_estado_equipa(self):
        try:
            ...
            if self.main_window.mostrar_confirmacion("Confirmar Actualización de Estado", f"¿Deseas mover {cantidad} unidades del equipo {num_equipo} desde el estado '{estado_origen}' al nuevo estado '{estado_nuevo}'?"):
                self.actualizar_estado_equipa(num_equipo, estado_nuevo, estado_origen, cantidad)
            else:
                QMessageBox.information(None, "Actualización Cancelada", "La operación de actualización del estado ha sido cancelada.")
        except ValueError as e:
            error_msg = str(e)
            if "invalid literal for int()" in error_msg:
                QMessageBox.warning(None, "Datos Inválidos", "Asegúrate de que los campos de numero y cantidad contengan valores numéricos enteros válidos.")
            elif "Campos de estado vacíos" in error_msg:
                 QMessageBox.warning(None, "Datos Faltantes", "Los campos Estado Nuevo' y Estado Origen' no pueden estar vacíos.")
            else:
                 QMessageBox.critical(None, "Error de Pre-Validación", f"Ocurrió un error inesperado al validar los datos: {e}")
        except Exception as e:
            QMessageBox.critical(None, "Error Inesperado", f"Ocurrió un error al procesar los datos: {e}")
