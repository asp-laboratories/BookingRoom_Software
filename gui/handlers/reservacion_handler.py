...
from PyQt6.QtWidgets import QMessageBox, QListWidgetItem, QLabel, QPushButton, QFrame, QHBoxLayout, QVBoxLayout
...
class ReservacionHandler:
    ...
    def registrar_reservacion(self):
        ...
        for equipa in equipas:
            equipa.equipamiento = self.equipamiento.obtener_codigo_equipamiento(equipa.equipamiento)
        ...
        respuesta = self.main_window.mostrar_confirmacion("Confirmacion de Reservacion", "¿Seguro de continuar con la resevacion?")
        if respuesta:
            self.reservacion.crear_reservacion(fechaReserE, fecha, hora_inicio, hora_fin, descripEvento, estimaAsistentes, tipo_montaje, rfcTrabajador, clienteReservacion, sali['nombre'], equipas, lista_servicios)
    ...
    def total_reservacion(self):
        ...
        total = subtotalServicios + subtotalEquipamiento + self.main_window.subtotal_salon
    ...
    def intentar_registrar_reservacion(self):
        try:
            ...
            equipas = self.generar_lista_equipamiento_reservado()
            for equipa in equipas:
                codigo_equipo = self.equipamiento.obtener_codigo_equipamiento(equipa.equipamiento)
            ...
            if self.main_window.mostrar_confirmacion(
                ...
                self.reservacion.crear_reservacion(
                    ...
                    estimaAsistentes, tipo_montaje, rfcTrabajador, self.main_window.clienteNombre, 
    ...
    def crear_control_cantidad(self, nombre, costoRenta, numEquipa):
        ...
        btn_menos.setStyleSheet("QPushButton{color: #ffffff; background-color: #000000;}") 
        ...
        if not slon_especifico == "Selecciona un salon":
            ...
            for datoSalon in montajes:
                ...
                for datosMobiliario in datoSalon['mobiliarios']:
                    if not self.mobiliario.stock_disponible(datosMobiliario.mobiliario, datosMobiliario.cantidad):
                        bol = False
                if bol:
