...
class RegistroCliente():
    def __init__(self):
        ...
        self.registro_cliente.clienteConfirmar.clicked.connect(self.intentar_registrar_cliente_completo)
    ...
    def registrar_cliente_ejecutar(self, rfc, nombre, priApellido, priAmater, nombreFiscal, correo, 
        ...
        try:
            ...
            resultado = cliente.registrar_clientes(
                rfc, nombre, priApellido, priAmater, nombreFiscal, correo, colonia, calle, numero, tipo_cliente
            )
    
            # 2. Registro de Teléfonos (solo si tienen contenido)
            if telefono1:
                telefono.registrar_telefono(telefono1, rfc, None)
            if telefono2 and self.registro_cliente.reTelefono2.isEnabled(): # Verifica si el campo está habilitado
                telefono.registrar_telefono(telefono2, rfc, None)
            if telefono3 and self.registro_cliente.reTelefono3.isEnabled(): # Verifica si el campo está habilitado
        ...
        try:
            ...
            if self.mostrar_confirmacion(
                ...
                self.registrar_cliente_ejecutar(
                    rfc, nombre, priApellido, priAmater, nombre_fiscal, correo, 
                    colonia, calle, numero, tipo_cliente, 
                    telefono1, telefono2, telefono3
                )
            else:
            ...
            if "Tipo Cliente Faltante" in error_type:
                 QMessageBox.warning(None, "Datos Faltantes", "Debes seleccionar si es Persona Física o Moral.")
            elif "RFC Inválido" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El RFC no es válido o es demasiado corto.")
            elif "Nombre Inválido" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El Nombre solo debe contener texto y debe tener al menos 2 caracteres.")
            elif "Apellido Paterno Inválido" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El Apellido Paterno es inválido (solo texto, min. 2 caracteres).")
            elif "Apellido Materno Inválido" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El Apellido Materno es inválido.")
            elif "Email Inválido" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El Correo Electrónico no es válido o es demasiado corto.")
            elif "Colonia Inválida" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "La Colonia no es válida (solo texto, min. 2 caracteres).")
            elif "Calle Inválida" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "La Calle no es válida (solo texto, min. 2 caracteres).")
            elif "Teléfono Principal Faltante" in error_type:
                QMessageBox.warning(None, "Datos Faltantes", "Debes ingresar el número de teléfono principal.")
            elif "Nombre Fiscal Faltante" in error_type:
                QMessageBox.warning(None, "Datos Faltantes", "El Nombre Fiscal es obligatorio para Personas Morales.")
            elif "invalid literal for int()" in error_type:
                QMessageBox.warning(None, "Datos Inválidos", "El campo 'Número' (de la calle) debe ser un número entero válido.")
            else:
