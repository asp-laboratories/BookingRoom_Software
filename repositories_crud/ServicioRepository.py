...
class ServicioRepository:
    ...
    def crear_servicio(
        self, servicio
    ):  # Para insertar los datos de un servicio a la base de datos se utiliza, una funcion en la cual contendra un parametro que
        ...
        try:  # Intentamos ejecutar el bloque try.
            ...
            """,
                (
                    ...
                    servicio.costo_renta,
                    servicio.tipo_servicio,
                ),
            )  # mediante la variable cursor podemos hacer
            ...
            print(
                "Se añadio un de servicio"
            )  # mensaje de confirmacion de que todo salio correcto.
            ...
            print(
                f"Error al crear un servicio: {error}"
            )  # Mediante el alias mostramos en pantalla el error.
    ...
    def listar_servicio(self):  # Metodo para traer todos los datos de una tabla.
        ...
        try:
            cursor = self.db.cursor(
                dictionary=True
            )  # Obtenemos el cursor para poder hacer consultas, pero convertimos el cursor a diccioario.
            cursor.execute(
                "SELECT * FROM servicio"
            )  # Hacemos la consulta para traer todos los servicios.
    ...
    def listar_servicio_buscar(
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM servicio WHERE numServicio = %s", (numServicio,)
            )  # Hacemos la consulta para traer todos los servicios.
            resultados = cursor.fetchall()  # Guardamos en una variable cursor.fetchall() que basicamente significa que trae todos los resultados existe otro que es
            # cursor.fetchone() que solo trae una solo fila es aplicable para casos donde se utilize un where.
    ...
    def obtener_servicios_inner(
        self,
    ):  # Metodo para traer mucha informacion en un inner join.
        if not self.db.conectar():
        ...
        try:
            ...
            results = (
                cursor.fetchall()
            )  # Misma explicacion de arriba, recibimos un diccionario.
            ...
            for row in results:  # Mecanica para poder meter los datos del diccionario en un objeto, hay que pensar como si el llamado de la clase este en posicion
                ...
                # datos del servicio.
                servicio.tipo_nombre = row[
                    "tipo"
                ]  # la clase Servicio tiene una propiedad que esta por fuera del constructor es decir que no se requiere en el llenado
                ...
                servicios.append(
                    servicio
                )  # Recordando el arreglo de arriba, guardamos el objeto/intancia servicio.
    ...
    def obtener_num_servicio(self, nombre):
        ...
        try:
            cursor = self.db.cursor()
            cursor.execute(
                "SELECT numServicio FROM servicio WHERE nombre like %s",
                (f"%{nombre}%",),
            )
