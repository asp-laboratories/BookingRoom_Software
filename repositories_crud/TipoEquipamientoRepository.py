...
class TipoEquipaRepository:
    ...
    def obtener_equipamentos_de_tipo(
        self, tipo_equipa
    ):  # Aca se mostrarian los equipamentos por tipo de equipamento
        if not self.db.conectar():
            return None
        ...
        try:
            ...
            cursor.execute(
                """SELECT * FROM tipo_equipa WHERE codigoTiEquipa = %s""",
                (tipo_equipa,),
            )
            ...
            tipoE = TipoEquipa(
                codigoTiEquipa=data_tipo["codigoTiEquipa"], descripcion=["descripcion"]
            )

            cursor.execute(
                """" SELECT * FROM equipamiento WHERE tipo_equipa = %s""",
                (tipo_equipa,),
            )
    ...
    def descripcion_de_tipo(self, t_descripcion):
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM tipo_equipa WHERE descripcion = %s", (t_descripcion,)
            )
