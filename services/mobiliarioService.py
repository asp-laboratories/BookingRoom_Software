...
class mobiliarioService:
    ...
    def registrar_mobiliario(
        self, nombre, costoRenta, stock, tipo_mob, caracteristicas
    ):
    ...
    def listar_mobiliarios(
        self,
    ):  # Sin descripcion detallada de tipo, estado, trabajador
        print("Mobiliarios: ")
        ...
        if info_mob:
            ...
            for row in info_mob:
                ...
                if caracteristica not in lista_apoyo_caracs:
                    ...
                    mob_carac = MobCarac(
                        nombreCarac=caracteristica, tipo_carac=tipo_carac
                    )
                ...
                if estado not in lista_apoyo_esta:
                    esta_mb = EstaMob(
                        codigoMob=row["codigoMob"], descripcion=row["descripcion"]
                    )
                    inventario = InventarioMob(
                        esta_mob=esta_mb, cantidad=row["cantidad"]
                    )
    ...
    def actu_datos_mob(self, numMob, new_nombre, new_costoRenta, new_stock):
        ...
        return self.mobiliario_repository.actu_mob_datos(
            numMob, new_nombre, new_costoRenta, new_stock
        )

    def actu_carac_mob(self, numCarac, nombreCarac, tipo_carac):
        print("Actualizando caracteristica del mobiliario")
        return self.mobiliario_repository.actu_mob_carac(
            numCarac, nombreCarac, tipo_carac
        )
    ...
    def datos_mob(self, numMob):
        ...
        for carac in info_mob:
            if carac["estado"] not in estados:
                ...
                estado = {"estado": carac["estado"], "cantidad": carac["cantidad"]}
    ...
    def stock_disponible(self, nombre, cantidad):
        ...
        disponiobles = self.mobiliario_repository.mobiliario_disponible(
            numMob["numMob"]
        )
        if disponiobles is None:
            return False
